...
def build_model_and_test(user_input):
    """
    Trains an ML model using PyCaret if it does not exist and always performs testing.
    """
    DATA_PATH = filepath()
    target_column = targeted_column(user_input)
    MODEL_DIR = f"./pycaret_model_{target_column}"
    ...
    if os.path.exists(MODEL_DIR):
        ...
        setup(data=df, target=LABEL_COLUMN, session_id=42)
    ...
    y_pred = predict_model(model, data=X_test)['Label']
...
def deploy_model(user_input):
    """
    Deploy the trained PyCaret model, make predictions, and visualize actual vs predicted values.
    """
    ...
    y_pred = predictions["Label"]  # PyCaret stores predicted labels under "Label"

    # Determine Task Type (Regression or Classification)
    num_classes = len(np.unique(y_actual))

    if num_classes > 10:  # Regression
...
def predict_custom_input(user_input):
    ...
    # Make prediction
    prediction = predict_model(predictor, data=input_df)

    print(f"\nPredicted Value: {prediction['Label'].values[0]}")
    return prediction['Label'].values[0]
